package com.atpl.mmg.AandA.filter;
...
import com.atpl.mmg.AandA.common.Status;
...
@ComponentScan(basePackages = { "com.atpl.mmg.AandA" })
...
public class MMGFilter implements Constants, Filter {
	...
	public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
		...
		String mobileNumber = null, role = null, deleteProfile = null, forgotPassword = null, boardingrequest = null,profileByprofileId=null;
		if (url.contains("swagger") || url.contains("api-docs")
			...
			mmgProperties.setxApiSource(xApiSource);
			...
			if (url.contains("/profile/forgotpassword/mobile/")) {
				forgotPassword = request.getRequestURI().substring(request.getContextPath().length());
			}

			if (url.contains("v2/boarding/request")) {
				boardingrequest = request.getRequestURI().substring(request.getContextPath().length());
			}
			if (url.contains("v2/profile/id/")) {
				profileByprofileId = request.getRequestURI().substring(request.getContextPath().length());
			}
			...
			if (jsonResponse == null && !request.getMethod().equalsIgnoreCase("OPTIONS")) {
				...
				if (null == xApiSource || xApiSource.isEmpty() || !Arrays.asList(skipToken).contains(xApiSource)) {
					...
					if (!"Yes".equalsIgnoreCase(xByPassToken))
						try {
							jsonResponse = validateApiToken(accessToken, requestPath,
									Arrays.asList("/v2/login", "/v1/session", "/v2/profile/sms/otp", "/v2/profile",
											mobileNumber, forgotPassword, role, deleteProfile, "/v2/profile/password",
											"/v1/viewDashboardcustomer", "/v1/session/last/access/time",
											"/v1/login/refreshToken", "/v2/profile/validateOtp",profileByprofileId, boardingrequest),
									response);
						} catch (Exception e) {
	...
	public String validateApiToken(String accessToken, String requestPath, List<String> exludePathList,
		...
		if (!exludePathList.contains(requestPath)) {
			Map<String, Object> httpResponse = GenericHttpClient.doHttpGet(
					mmgProperties.getOauthUrl() + "v1/oauth/loadAuthentication", ApiUtility.getHeaders("", accessToken),
					MmgEnum.OAUTH.name());
